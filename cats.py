...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    y = [x for x in paragraphs if select(x)]
    if k >= len(y):
        return ''
    else:
        return y[k]
...
def about(topic):
    ...
    def inParagraph(paragraph):
        ...
        return bool([x for x in paragraph if remove_punctuation(lower(x)) in topic])
    return inParagraph
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    if len(typed_words) < len(source_words):
        amount_of_words = len(typed_words)
    else:
        amount_of_words = len(source_words)
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        x = min(word_list, key = lambda k: diff_function(typed_word, k, limit))
...
def feline_fixes(typed, source, limit):
    ...
    if (not typed) or (not source):
        ...
        if len(typed) < len(source):
            difference = len(source) - len(typed)
            return feline_fixes(typed, source[:len(typed)], limit - difference) + difference
        elif len(typed) > len(source):
            difference = len(typed) - len(source)
            return feline_fixes(typed[:len(source)], source, limit - difference) + difference
        else:
            if typed[-1] == source[-1]:
                return feline_fixes(typed[:len(typed)-1], source[:len(source)-1], limit)
            else:
                return feline_fixes(typed[:len(typed)-1], source[:len(source)-1], limit - 1) + 1
    # END PROBLEM 6
